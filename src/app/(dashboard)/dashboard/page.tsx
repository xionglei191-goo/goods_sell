...
type MetricCard = {
  ...
  tone: "transaction" | "growth" | "customer" | "risk";
};
...
const toneClasses = {
  transaction: "bg-[#fff1e8] text-[#b9472d] ring-[#f8d6c9]",
  growth: "bg-emerald-50 text-emerald-600 ring-emerald-100",
  customer: "bg-[#fff1e8] text-[#b9472d] ring-[#f8d6c9]",
  risk: "bg-red-50 text-red-600 ring-red-100",
};
...
function fallbackDashboardData(): DashboardData {
  return {
    metrics: [
      { title: "今日订单数", value: "18", trend: 12, icon: ShoppingBag, tone: "transaction" },
      { title: "今日销售额", value: formatCurrency(32480), trend: 8, icon: Banknote, tone: "growth" },
      { title: "新增客户数", value: "6", trend: -3, icon: Users, tone: "customer" },
      { title: "待处理事项", value: "14", trend: 5, icon: ClipboardList, tone: "risk" },
    ],
...
async function getDashboardData(): Promise<DashboardData> {
  try {
    ...
    return {
      metrics: [
        { title: "今日订单数", value: todayOrders.toString(), trend: orderTrend, icon: ShoppingBag, tone: "transaction" },
        { title: "今日销售额", value: formatCurrency(todaySalesAmount), trend: salesTrend, icon: Banknote, tone: "growth" },
        { title: "新增客户数", value: newCustomers.toString(), trend: customerTrend, icon: Users, tone: "customer" },
        { title: "待处理事项", value: pendingCount.toString(), trend: pendingCount, icon: ClipboardList, tone: "risk" },
      ],
...
export default async function DashboardPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold text-neutral-950">仪表盘</h1>
        <p className="mt-1 text-sm text-neutral-500">实时掌握订单、销售、库存和回款情况</p>
      </div>
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {data.metrics.map((metric) => {
          ...
          return (
            <div className="dashboard-data-card p-5" key={metric.title}>
              <div className="flex items-start justify-between gap-4">
                ...
                <span className={cn("flex h-11 w-11 items-center justify-center rounded-md ring-1", toneClasses[metric.tone])}>
      ...
      <section className="grid gap-6 xl:grid-cols-3">
        <div className="dashboard-data-card p-5">
          <div className="mb-4 flex items-center justify-between">
            <h2 className="text-lg font-semibold text-neutral-950">库存预警 TOP10</h2>
            ...
            <table className="w-full min-w-96 text-left text-sm">
              ...
              <tbody>
                {data.lowStock.map((item) => (
                  <tr className="border-b border-neutral-100 last:border-0" key={item.name}>
                    <td className="py-3 text-neutral-950">{item.name}</td>
                    <td className="py-3 font-medium text-red-600">{item.stock}</td>
                    <td className="py-3 text-neutral-600">{item.safeStock}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="dashboard-data-card p-5">
          <h2 className="mb-4 text-lg font-semibold text-neutral-950">应收账款 TOP10</h2>
          <div className="overflow-x-auto">
            <table className="w-full min-w-96 text-left text-sm">
              <thead className="text-neutral-500">
                <tr className="border-b border-neutral-100">
              ...
              <tbody>
                {data.receivables.map((item) => (
                  <tr className="border-b border-neutral-100 last:border-0" key={item.customer}>
                    <td className="py-3 text-neutral-950">{item.customer}</td>
                    <td className="py-3 font-medium text-neutral-950">{formatCurrency(item.amount)}</td>
                    <td className={cn("py-3", item.overdueDays > 30 ? "text-red-600" : "text-neutral-600")}>{item.overdueDays} 天</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="dashboard-data-card p-5">
          <h2 className="mb-4 text-lg font-semibold text-neutral-950">最近订单</h2>
          <div className="space-y-3">
            {data.recentOrders.map((order) => (
              <div className="rounded-md border border-neutral-100 p-3" key={order.orderNo}>
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="truncate text-sm font-medium text-neutral-950">{order.orderNo}</p>
                    <p className="mt-1 text-sm text-neutral-500">{order.customer}</p>
                  </div>
                  <span className="rounded-full bg-neutral-100 px-2 py-1 text-xs text-neutral-600">{order.status}</span>
                </div>
                <div className="mt-3 flex items-center justify-between text-sm">
                  <span className="font-medium text-neutral-950">{formatCurrency(order.amount)}</span>
