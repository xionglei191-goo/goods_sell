          ...
          <table className="w-full min-w-[1100px] text-left text-sm">
            ...
            <tbody>
              {data.items.map((inquiry) => (
                <tr className="border-t border-slate-100 hover:bg-slate-50" key={inquiry.id}>
...
function SummaryCard({ label, value, tone = "slate" }: { label: string; value: string; tone?: "slate" | "purple" | "emerald" }) {
  const color = tone === "purple" ? "text-purple-700" : tone === "emerald" ? "text-emerald-700" : "text-slate-900";
  return (
