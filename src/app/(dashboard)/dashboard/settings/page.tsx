    ...
    <div className="space-y-6">
      <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
        <div>
          ...
          <p className="mt-1 text-sm text-neutral-500">业务参数、用户管理入口、集成配置状态和商城基础设置。</p>
        </div>
        <Button asChild>
      ...
      <section id="integrations" className="surface-panel p-5">
        <h2 className="font-semibold text-neutral-950">集成配置状态</h2>
...
function IntegrationCard({ icon: Icon, label, enabled, mutedText }: { icon: typeof Users; label: string; enabled: boolean; mutedText?: string }) {
  return (
    <div className={enabled ? "rounded-md border border-emerald-200 bg-emerald-50 p-4" : "rounded-md border border-amber-200 bg-amber-50 p-4"}>
      ...
      <p className="mt-1 text-sm text-neutral-600">{enabled ? "已配置" : mutedText ?? "未配置"}</p>
    </div>
  );
}
