  ...
  return (
    <div className="space-y-6">
      <div className="dashboard-page-heading">
        <div>
          <h1 className="text-2xl font-semibold text-neutral-950">产品管理</h1>
          <p className="mt-1 text-sm text-neutral-500">维护产品资料、分类、品牌和大单分单阈值</p>
        </div>
        <Button asChild>
          ...
          <table className="w-full min-w-[960px] text-left text-sm">
            ...
            <tbody>
              {products.items.map((product) => (
                <tr className="dashboard-table-row" key={product.id}>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      <div className="flex h-12 w-12 items-center justify-center rounded-md border border-orange-100 bg-[#fff1e8] text-xs text-[#b9472d]">暂无图片</div>
                      <div>
                        <p className="font-medium text-neutral-950">{product.name}</p>
                        <p className="mt-1 text-xs text-neutral-500">大单阈值：{product.bulkThreshold}</p>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="px-4 py-3 money-muted">{formatCurrency(product.retailPrice)}</td>
                  <td className={cn("px-4 py-3", product.stock <= 0 ? "metric-risk" : product.stock <= 30 ? "metric-warning" : "metric-positive")}>{product.stock}</td>
                  <td className="px-4 py-3">
