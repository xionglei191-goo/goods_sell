  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <div className="flex h-11 overflow-hidden rounded-md border border-orange-100 bg-[var(--shop-control)]">
          <button aria-label="减少数量" className="flex w-11 items-center justify-center text-neutral-600 hover:bg-[#fff8f6] disabled:text-stone-300" disabled={quantity <= 1} onClick={() => changeQuantity(quantity - 1)} type="button">
            <Minus className="h-4 w-4" />
          </button>
          <input
            className="w-14 border-x border-orange-100 bg-[var(--shop-control)] text-center outline-none"
            max={stock}
            min={1}
            ...
            value={quantity}
          />
          <button aria-label="增加数量" className="flex w-11 items-center justify-center text-neutral-600 hover:bg-[#fff8f6] disabled:text-stone-300" disabled={quantity >= stock} onClick={() => changeQuantity(quantity + 1)} type="button">
            <Plus className="h-4 w-4" />
          </button>
        </div>
        <span className="text-xs text-neutral-400">库存 {stock}</span>
      </div>
