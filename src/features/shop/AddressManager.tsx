  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-neutral-950">地址管理</h1>
          <p className="mt-1 text-sm text-neutral-500">配送范围限制为湖南省湘潭市</p>
        </div>
      ...
      {isEditing ? (
        <section className="grid gap-3 shop-block-card p-4 sm:grid-cols-2">
          <input className="shop-form-input h-10" onChange={(event) => setInput((current) => ({ ...current, name: event.target.value }))} placeholder="收货人" value={input.name} />
          <input className="shop-form-input h-10" onChange={(event) => setInput((current) => ({ ...current, phone: event.target.value }))} placeholder="联系电话" value={input.phone} />
          <select className="shop-form-input h-10" onChange={(event) => setInput((current) => ({ ...current, district: event.target.value }))} value={input.district}>
          ...
          <input className="shop-form-input h-10" onChange={(event) => setInput((current) => ({ ...current, detail: event.target.value }))} placeholder="详细地址" value={input.detail} />
          <label className="flex items-center gap-2 text-sm text-neutral-600 sm:col-span-2">
            <input checked={input.isDefault} className="h-4 w-4 accent-red-800" onChange={(event) => setInput((current) => ({ ...current, isDefault: event.target.checked }))} type="checkbox" />
            设为默认地址
          </label>
      ...
      <div className="grid gap-3 md:grid-cols-2">
        {addresses.map((address) => (
          <article className="shop-block-card p-4" key={address.id}>
            <div className="flex items-start gap-3">
              <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-red-50 commerce-accent">
                <MapPin className="h-5 w-5" />
              </span>
              <div className="min-w-0 flex-1">
                <p className="font-semibold text-neutral-950">
                  {address.name} {address.phone}
                  {address.isDefault ? <span className="ml-2 rounded-full bg-red-50 px-2 py-0.5 text-xs commerce-accent">默认</span> : null}
                </p>
                <p className="mt-1 text-sm text-neutral-600">{address.province}{address.city}{address.district}{address.detail}</p>
              </div>
            </div>
      ...
      {addresses.length === 0 ? <div className="shop-empty-state">暂无地址，请新增湘潭市收货地址。</div> : null}
    </div>
  );
}
