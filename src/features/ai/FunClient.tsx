  ...
  return (
    <div className="grid gap-5 lg:grid-cols-[1fr_320px]">
      <section className="rounded-lg bg-white p-5 shadow-sm ring-1 ring-stone-200">
        <h2 className="text-xl font-bold text-stone-950">你是什么酒的性格？</h2>
        <div className="mt-5 space-y-5">
          {personalityQuestions.map((question, index) => (
            <div key={question.id}>
              <p className="font-semibold text-stone-900">{index + 1}. {question.question}</p>
              <div className="mt-2 grid gap-2 sm:grid-cols-3">
                {question.options.map((option) => (
                  <button
                    className={answers[question.id] === option ? "rounded-md border border-red-200 bg-red-50 px-3 py-2 text-sm font-medium text-[#dc2626]" : "rounded-md border border-stone-200 px-3 py-2 text-sm text-stone-600 hover:bg-stone-50"}
                    key={option}
        ...
        {testMessage ? <p className="mt-3 text-sm text-stone-500">{testMessage}</p> : null}
        {result ? (
          <div className="mt-5 rounded-lg bg-red-50 p-4">
            <h3 className="text-lg font-bold text-[#dc2626]">{result.title}</h3>
            <p className="mt-2 text-sm leading-6 text-stone-700">{result.description}</p>
      ...
      <aside className="space-y-4">
        <section className="rounded-lg bg-white p-5 shadow-sm ring-1 ring-stone-200">
          <h2 className="text-lg font-bold text-stone-950">每日签到</h2>
          <p className="mt-1 text-sm text-stone-500">1天5分，连续7天50分，连续30天200分</p>
          ...
          {checkInMessage ? <p className="mt-3 text-sm text-stone-500">{checkInMessage}</p> : null}
          <div className="mt-4 grid grid-cols-5 gap-2">
            {(checkIn?.recent ?? []).slice(0, 10).map((item) => (
              <div className="rounded-md bg-stone-50 px-2 py-2 text-center text-xs" key={item.date}>
                <p className="font-semibold text-stone-900">{item.streak}</p>
                <p className="text-stone-400">+{item.points}</p>
              </div>
            ))}
          </div>
        </section>
