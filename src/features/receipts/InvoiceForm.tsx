  ...
  return (
    <section className="surface-panel p-5">
      <div className="flex items-center gap-2">
        <FilePlus2 className="h-5 w-5 text-orange-700" />
        <h2 className="font-semibold text-neutral-950">开具电子发票</h2>
      </div>
      <div className="mt-4 grid gap-3 md:grid-cols-2">
        <select className="form-input md:col-span-2" onChange={(event) => changeOrder(event.target.value)} value={orderId}>
          {orders.map((order) => (
        ...
        <select className="form-input" onChange={(event) => setType(event.target.value as typeof type)} value={type}>
