...
export function StockMovementForm({ mode, products }: StockMovementFormProps) {
  ...
  return (
    <form className="max-w-2xl space-y-5 surface-panel p-5" onSubmit={form.handleSubmit(submit)}>
      <label className="block">
        <span className="mb-2 block text-sm font-medium text-neutral-700">搜索产品</span>
        <input className="form-input" onChange={(event) => setQuery(event.target.value)} placeholder="输入产品名称或 SKU" value={query} />
      </label>

      <label className="block">
        <span className="mb-2 block text-sm font-medium text-neutral-700">选择产品</span>
      ...
      <label className="block">
        <span className="mb-2 block text-sm font-medium text-neutral-700">{mode === "in" ? "入库数量" : "出库数量"}</span>
