  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[360px_1fr]">
      <section className="rounded-lg bg-white p-5 shadow-sm ring-1 ring-slate-200">
        <h2 className="text-lg font-semibold text-slate-900">新增品牌</h2>
        <div className="mt-5 space-y-4">
          <label className="block">
            <span className="mb-2 block text-sm font-medium text-slate-700">品牌名称</span>
            <input className="form-input" onChange={(event) => setName(event.target.value)} placeholder="如：茅台" value={name} />
          </label>
          <label className="block">
            <span className="mb-2 block text-sm font-medium text-slate-700">品牌描述</span>
            <textarea
      ...
      <section className="rounded-lg bg-white p-5 shadow-sm ring-1 ring-slate-200">
        <h2 className="text-lg font-semibold text-slate-900">品牌列表</h2>
        <div className="mt-5 overflow-x-auto">
          <table className="w-full min-w-[560px] text-left text-sm">
            ...
            <tbody>
              {brands.map((brand) => (
                <tr className="border-b border-slate-100 last:border-0" key={brand.id}>
                  <td className="py-3 font-medium text-slate-900">{brand.name}</td>
                  <td className="py-3 text-slate-600">{brand.description ?? "-"}</td>
