  ...
  return (
    <div className="grid gap-5 lg:grid-cols-[1fr_320px]">
      <section className="surface-panel p-5">
        <h2 className="text-lg font-semibold text-neutral-950">待核销订单</h2>
        <div className="mt-4 space-y-3">
          {data.orders.map((order) => (
            <div className={cn("grid gap-3 rounded-md border p-3 md:grid-cols-[1fr_150px]", order.overdue ? "border-orange-200 bg-orange-50" : "border-[var(--dashboard-line)]")} key={order.id}>
              <div>
                <p className="font-medium text-neutral-950">{order.orderNo}</p>
                <p className="mt-1 text-sm text-neutral-500">
                  {formatDate(order.createdAt)} · 应收 {formatCurrency(order.payableAmount)} · 已收 {formatCurrency(order.paidAmount)}
                </p>
                {order.overdue ? <p className="mt-1 text-xs font-medium text-orange-700">账期超过 30 天</p> : null}
              </div>
              <input
          ...
          {data.orders.length === 0 ? <div className="rounded-md bg-orange-50 px-4 py-12 text-center text-sm text-neutral-500">该客户暂无待核销订单</div> : null}
        </div>
      </section>

      <aside className="space-y-4 lg:sticky lg:top-24 lg:self-start">
        <section className="surface-panel p-5">
          <h2 className="text-lg font-semibold text-neutral-950">登记收款</h2>
          <div className="mt-4 space-y-3">
            ...
            <select className="form-input h-11" onChange={(event) => setMethod(event.target.value as typeof method)} value={method}>
              <option value="TRANSFER">转账</option>
              <option value="CASH">现金</option>
              <option value="WECHAT">微信</option>
            </select>
          </div>
          {message ? <p className="mt-3 rounded-md bg-neutral-100 px-3 py-2 text-sm text-neutral-600">{message}</p> : null}
          <div className="mt-4 flex items-center justify-between border-t border-neutral-100 pt-4">
            <span className="text-sm text-neutral-500">本次收款</span>
